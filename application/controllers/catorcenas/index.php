...
require_once(APPPATH.ENTITY_METADATA_CATORCENA);
...
class index extends generic_controller {
    ...
    function __construct() {
        parent::__construct();
        $this->load->model('catorcenas/catorcena_model');
    }
     ...
     *
     * @RequiresPermission ["catorcenas", "R"]
     *
     */
    public function index_get()
    {
        ...
        try {
            $result = $this->catorcena_model->getAll($this->get(), $this->session->fk_pais, $offset, $limit, $sort, $pagination);
            if ($result["pagination"])
                $this->response(array('result' => 'OK', 'pagination' => $result["pagination"], 'catorcenas' => $result["result"]), 200);
            else
                $this->response(array('result' => 'OK', 'catorcenas' => $result["result"]), 200);
    ...
    /**
     *
     * @RequiresPermission ["catorcenas", "C"]
     *
     */
    function index_post()
    {
        ...
        try {
            ...
            $result = $this->catorcena_model->create($entity, $array, $this->session->fk_pais);
            if ($result)
     ...
     *
     * @RequiresPermission ["catorcenas", "U"]
     *
     */
    function index_put()
    {
        ...
        try {
            ...
            $result = $this->catorcena_model->update($this->get(), $this->put(), $this->session->fk_pais);
            if ($result)
                $this->response(array('result' => 'OK'), 200);
            else {
                $err = new APIerror(ERROR_SAVING_DATA);
    ...
    /**
     *
     * @RequiresPermission ["catorcenas", "D"]
     *
     */
    function index_delete()
    {
        ...
        try {
            ...
            $result = $this->catorcena_model->delete($get_vars, $this->session->fk_pais);
            if ($result)
                $this->response(array('result' => 'OK'), 200);
            else {
                $err = new APIerror(ERROR_SAVING_DATA);
