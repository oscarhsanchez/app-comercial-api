...
require_once(APPPATH.ENTITY_PARAMETRO_TPV);
...
class index extends generic_controller {
    ...
    function __construct() {
        parent::__construct();
        $this->load->model('parametros/parametro_model');
    }
     ...
     *
     * @RequiresPermission ["parametros", "R"]
     *
     */
    public function index_get()
    {
        ...
        try {
            $result = $this->parametro_model->getAll($this->get(), $this->session->fk_pais, $offset, $limit, $sort, $pagination);
            if ($result["pagination"])
                $this->response(array('result' => 'OK', 'pagination' => $result["pagination"], 'parametros' => $result["result"]), 200);
            else
                $this->response(array('result' => 'OK', 'parametros' => $result["result"]), 200);
    ...
    /**
     *
     * @RequiresPermission ["parametros", "C"]
     *
     */
    function index_post()
    {
        ...
        try {
            ...
            $result = $this->parametro_model->create($entity, $array, $this->session->fk_pais);
            if ($result)
     ...
     *
     * @RequiresPermission ["parametros", "U"]
     *
     */
    function index_put()
    {
        ...
        try {
            ...
            $result = $this->parametro_model->update($get_vars, $this->put(), $this->session->fk_pais);
            if ($result)
                $this->response(array('result' => 'OK'), 200);
            else {
                $err = new APIerror(ERROR_SAVING_DATA);
    ...
    /**
     *
     * @RequiresPermission ["parametros", "D"]
     *
     */
    function index_delete()
    {
        ...
        try {
            ...
            $result = $this->parametro_model->delete($get_vars, $this->session->fk_pais);
            if ($result)
                $this->response(array('result' => 'OK'), 200);
            else {
                $err = new APIerror(ERROR_SAVING_DATA);
