...
require_once(APPPATH.ENTITY_METADATA_VENUE_FQ);



class foursquare_venues extends generic_controller {
   	
    function __construct() {
        parent::__construct();
        $this->load->model('metadata/fq_venue_model');
    }
    ...
    public function index_get()
    {
        ...
        try {
            $result = $this->fq_venue_model->getAll($this->get(), $this->session->fk_pais, $offset, $limit, $sort, $pagination);
            if ($result["pagination"])
                $this->response(array('result' => 'OK', 'pagination' => $result["pagination"], 'venues' => $result["result"]), 200);
            else
                $this->response(array('result' => 'OK', 'venues' => $result["result"]), 200);
    ...
    function index_post()
    {
        ...
        try {
            ...
            $result = $this->fq_venue_model->create($entity, $array, $this->session->fk_pais);
            if ($result)
    ...
    function index_put()
    {
        ...
        try {
            ...
            $result = $this->fq_venue_model->update($this->get(), $this->put(), $this->session->fk_pais);
            if ($result)
                $this->response(array('result' => 'OK'), 200);
            else {
                $err = new APIerror(ERROR_SAVING_DATA);
    ...
    function index_delete()
    {
        ...
        try {
            ...
            $result = $this->fq_venue_model->delete($get_vars, $this->session->fk_pais);
            if ($result)
                $this->response(array('result' => 'OK'), 200);
            else {
                $err = new APIerror(ERROR_SAVING_DATA);
