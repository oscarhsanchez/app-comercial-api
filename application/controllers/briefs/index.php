...
class index extends generic_controller {
    ...
    function __construct() {
        parent::__construct();
        $this->load->model('briefs/brief_model');
    }
    ...
    public function index_get()
    {
        ...
        try {
            $result = $this->brief_model->getAll($this->get(), $this->session->fk_pais, $offset, $limit, $sort, $pagination);
            if ($result["pagination"])
                $this->response(array('result' => 'OK', 'pagination' => $result["pagination"], 'briefs' => $result["result"]), 200);
            else
                $this->response(array('result' => 'OK', 'briefs' => $result["result"]), 200);
    ...
    function index_post()
    {
        ...
        try {
            ...
            $result = $this->brief_model->create($entity, $array, $this->session->fk_pais);
            if ($result)
    ...
    function index_put()
    {
        ...
        try {
            ...
            $result = $this->brief_model->update($get_vars, $this->put(), $this->session->fk_pais);
            if ($result)
                $this->response(array('result' => 'OK'), 200);
            else {
                $err = new APIerror(ERROR_SAVING_DATA);
    ...
    function index_delete()
    {
        ...
        try {
            ...
            $result = $this->brief_model->delete($get_vars, $this->session->fk_pais);
            if ($result)
                $this->response(array('result' => 'OK'), 200);
            else {
                $err = new APIerror(ERROR_SAVING_DATA);
