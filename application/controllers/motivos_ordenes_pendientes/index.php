...
class index extends generic_controller {
    ...
    function __construct() {
        parent::__construct();
        $this->load->model('ordenes_trabajo/motivo_model');
    }
    ...
    public function index_get()
    {
        ...
        try {
            $result = $this->motivo_model->getAll($this->get(), $this->session->fk_pais, $offset, $limit, $sort, $pagination);
            if ($result["pagination"])
                $this->response(array('result' => 'OK', 'pagination' => $result["pagination"], 'motivos' => $result["result"]), 200);
            else
                $this->response(array('result' => 'OK', 'motivos' => $result["result"]), 200);
    ...
    function index_post()
    {
        ...
        try {
            ...
            $result = $this->motivo_model->create($entity, $array, $this->session->fk_pais);
            if ($result)
    ...
    function index_put()
    {
        ...
        try {
            ...
            $result = $this->motivo_model->update($get_vars, $this->put(), $this->session->fk_pais);
            if ($result)
                $this->response(array('result' => 'OK'), 200);
            else {
                $err = new APIerror(ERROR_SAVING_DATA);
    ...
    function index_delete()
    {
        ...
        try {
            ...
            $result = $this->motivo_model->delete($get_vars, $this->session->fk_pais);
            if ($result)
                $this->response(array('result' => 'OK'), 200);
            else {
                $err = new APIerror(ERROR_SAVING_DATA);
