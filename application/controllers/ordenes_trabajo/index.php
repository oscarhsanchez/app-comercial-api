...
require_once(APPPATH.ENTITY_BRIEF);
...
class index extends generic_controller {
    ...
    function __construct() {
        parent::__construct();
        $this->load->model('ordenes_trabajo/orden_trabajo_model');
    }
     ...
     *
     * @RequiresPermission ["ordenes_trabajo", "R"]
     *
     */
    public function index_get()
    {
        ...
        try {
            $result = $this->orden_trabajo_model->getAllOrdenes($this->get(), $this->session->fk_pais, $offset, $limit, $sort, $pagination);
            if ($result["pagination"])
                $this->response(array('result' => 'OK', 'pagination' => $result["pagination"], 'ordenes' => $result["result"]), 200);
            else
                $this->response(array('result' => 'OK', 'ordenes' => $result["result"]), 200);
    ...
    /**
     *
     * @RequiresPermission ["ordenes_trabajo", "C"]
     *
     */
    function index_post()
    {
        ...
        try {
            ...
            $result = $this->orden_trabajo_model->create($entity, $array, $this->session->fk_pais);
            if ($result)
     ...
     *
     * @RequiresPermission ["ordenes_trabajo", "U"]
     *
     */
    function index_put()
    {
        ...
        try {
            ...
            $result = $this->orden_trabajo_model->update($get_vars, $this->put(), $this->session->fk_pais);
            if ($result)
                $this->response(array('result' => 'OK'), 200);
            else {
                $err = new APIerror(ERROR_SAVING_DATA);
    ...
    /**
     *
     * @RequiresPermission ["ordenes_trabajo", "D"]
     *
     */
    function index_delete()
    {
        ...
        try {
            ...
            $result = $this->orden_trabajo_model->delete($get_vars, $this->session->fk_pais);
            if ($result)
                $this->response(array('result' => 'OK'), 200);
            else {
                $err = new APIerror(ERROR_SAVING_DATA);
