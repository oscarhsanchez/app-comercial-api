...
abstract class BaseFacebook
{
  ...
  /**
   * The ID of the Facebook usuarios, or 0 if the usuarios is logged out.
   *
   * @var integer
   */
  protected $user;
  ...
  public function setExtendedAccessToken() {
    try {
      ...
      // most likely that usuarios very recently revoked authorization.
   ...
   * to either a valid usuarios access token, or it's set to the application
   * access token if a valid usuarios access token wasn't available.  Subsequent
  ...
  /**
   * Determines and returns the usuarios access token, first using
   * the signed request if present, and then falling back on
   * the authorization code if present.  The intent is to
   * return a valid usuarios access token, or false if one is determined
   * to not be available.
   *
   * @return string A valid usuarios access token, or false if one
  ...
  /**
   * Get the UID of the connected usuarios, or 0
   * if the Facebook usuarios is not connected.
   *
  ...
  /**
   * Determines the connected usuarios by first examining any signed
   * requests, then considering an authorization code, and then
   * falling back to any persistent store storing the usuarios.
   *
   * @return integer The id of the connected Facebook usuarios,
   *                 or 0 if no such usuarios exists.
   */
  protected function getUserFromAvailableData() {
    ...
    if ($signed_request) {
      ...
      // if the signed request didn't present a usuarios id, then invalidate
    ...
    // use access_token to fetch usuarios id if we have a usuarios access_token, or if
   ...
   * potentially houses the signed request for the app usuarios.
   ...
   * to retrieve usuarios information and then extract
   * the usuarios ID.
   *
   * @return integer Returns the UID of the Facebook usuarios, or 0
   *                 if the Facebook usuarios could not be determined.
   */
   ...
   * usuarios when no authorization code is available.
   *
   * @return string The application access token, useful for gathering
   *                public information about usuarios and applications.
   */
   ...
   * a specific usuarios).  The authorization code is sent to graph.facebook.com
   * and a legitimate access token is generated provided the access token
   * and the usuarios for which it was generated all match, and the usuarios is
