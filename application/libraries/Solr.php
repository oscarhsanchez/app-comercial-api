...
class MY_Solr extends Apache_Solr_Service {
     ...
     * From
     *
     * This is the same as the MySQL from statement.
     * Modificado por Jaime para que coja el from como la coleccion
    ...
    final public function where($key, $value = null) {
        ...
        if(count($key) > 0) {
            foreach($key as $k => $v) {
                ...
                $query = $arrQuery['prefix'].$arrQuery['column'].':'.$v;

                $queryClass = new stdClass();
                $queryClass->query = $query;
                $queryClass->type = 'AND';

                $this->_arrWhere[] = $queryClass;
     ...
     * @param mixed $value
     */
    final public function where_or($key, $value = null) {
        if(!is_array($key)) {
            $key = array($key => $value);
        }
     ...
     * @return object
     */
    final public function where_range($field, $from, $to) {
        /* Get the operator */
     ...
     * @return object
     */
    final public function where_range_or($field, $from, $to) {
        /* Get the operator */
        $arrQuery = $this->_operator($field);
    ...
    final protected function _operator($query) {
        ...
        if(preg_match('/(\w+)(\s+)(.+)/', $query, $operator)) {
            ...
            if(array_key_exists($operator, $arrOpSwap)) {
                ...
                $query = trim(preg_replace("/{$operator}/", '', $query));
            }
        }
