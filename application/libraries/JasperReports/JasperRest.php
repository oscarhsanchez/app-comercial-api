...
class JasperRest {
    ...
    public function __construct($host, $cookiePath, $ssl = false) {
        $this->cookieFile = $cookiePath.'jasper_rest_cookies.txt';
    ...
    private function getRequestHandle($url) {
        ...
        try {
            $full_url = 'http://' . $this->host . $url;

//            echo  "Preparando peticion a : $full_url <br>";
...
//            throw new JasperException("{$this->getErrorMsg($head['http_code'])} on GET request ({$url})", $head['http_code'], $body);
            $body = $this->getErrorMsg($head['http_code']);
        }
    ...
    public function post($url, $post=null, $arrHeaders=array()) {
        try {
            $curl = $this->getRequestHandle($url);
            curl_setopt($curl, CURLOPT_POST, true);
            curl_setopt($curl, CURLOPT_POSTFIELDS, $post);
            //curl_setopt($curl, CURLINFO_HEADER_OUT, true);
            curl_setopt($curl, CURLOPT_HTTPHEADER, array_merge($arrHeaders, array(
                'Accept: application/json'
            )));
...
//            throw new JasperException("{$this->getErrorMsg($head['http_code'])} on POST request ({$url})", $head['http_code'], $body);
    ...
    public function putDVL($url, Array $arg = null) {
        try {
            $curl = $this->getRequestHandle($url);
