...
class esocialrestclient {
    private $host;
    private $ssl = false;
    
    private $cookieFile = '/tmp/rest_cookies';
    private $userAgent  = 'eSocial_rest_client.php/0.1';
    ...
    public function get($url) {
        ...
        if ($head['http_code'] >= 400) {
            throw new APIexception("{$this->getErrorMsg($head['http_code'])} on GET request ({$url})", $head['http_code'], $body);
        }
    ...
    public function post($url, $arg = null, $contentType = null) {
        try {
            $curl = $this->getRequestHandle($url);
        ...
        if ($head['http_code'] >= 400) {
            throw new APIexception("{$this->getErrorMsg($head['http_code'])} on POST request ({$url})", $head['http_code'], $body);
        }
    ...
    public function put($url, $fileString, $isBinary = false) {
        try {
            ...
            if ($streamBuffer === false) {
                throw new APIexception('failed to open PHP stream buffer');
            }
        ...
        if ($head['http_code'] >= 400) {
            throw new APIexception("{$this->getErrorMsg($head['http_code'])} on multipart PUT request ({$url})", $head['http_code'], $body);
        }
    ...
    public function multiput($url, $resourceDescriptorXml, $fileUri, $fileContent, $isBinary = true) {
        ...
        if ($head['http_code'] >= 400) {
            throw new APIexception("{$this->getErrorMsg($head['http_code'])} on PUT request ({$url})", $head['http_code'], $body);
        }
    ...
    public function delete($url) {
        ...
        if ($head['http_code'] >= 400) {
            throw new APIexception("{$this->getErrorMsg($head['http_code'])} on DELETE request ({$url})", $head['http_code'], $body);
        }
