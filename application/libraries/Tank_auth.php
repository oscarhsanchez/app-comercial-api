...
class Tank_auth
{
	...
	/**
	 * Check if usuarios logged in. Also test if usuarios is activated or not.
	 *
	 * @param	bool
	 * @return	bool
	 */
	...
	/**
	 * Create new usuarios on the site and return some data about it:
	...
	/**
	 * Change email for activation and return some data about usuarios:
	 * user_id, username, email, new_email_key.
	 * Can be called for not activated usuarios only.
	 *
	 * @param	string
	 * @return	array
	 */
	...
	/**
	 * Activate usuarios using given key
	 *
	 * @param	string
	 * @param	string
	 * @param	bool
	...
	/**
	 * Set new password key for usuarios and return some data about usuarios:
	 * user_id, username, email, new_pass_key.
	 * The password key can be used to verify usuarios when resetting his/her password.
	 *
	 * @param	string
	 * @return	array
	 */
	...
	/**
	 * Check if given password key is valid and usuarios is authenticated.
	 *
	 * @param	string
	 * @param	string
	 * @return	bool
	...
	/**
	 * Replace usuarios password (forgotten) with a new one (set by usuarios)
		...
		if ((strlen($user_id) > 0) AND (strlen($new_pass_key) > 0) AND (strlen($new_password) > 0)) {
			...
			if (!is_null($user = $this->ci->users->get_user_by_id($user_id, TRUE))) {
				...
				if ($this->ci->users->reset_password(
					...
					// Clear all usuarios's autologins
	...
	/**
	 * Change usuarios password (only when usuarios is logged in)
	 *
	 * @param	string
	 * @param	string
	 * @return	bool
	...
	/**
	 * Change usuarios email (only when usuarios is logged in) and return some data about usuarios:
	...
	/**
	 * Delete usuarios from the site (only when usuarios is logged in)
	 *
	 * @param	string
	 * @return	bool
	 */
	...
	/**
	 * Save data for usuarios's autologin
	 *
	 * @param	int
	 * @return	bool
	 */
	...
	/**
	 * Clear usuarios's autologin data
	 *
	 * @return	void
	 */
	...
	/**
	 * Login usuarios automatically if he/she provides correct autologin verification
	 *
	 * @return	void
	 */
	private function autologin()
	{
		if (!$this->is_logged_in() AND !$this->is_logged_in(FALSE)) {			// not logged in (as any usuarios)
			...
			if ($cookie = get_cookie($this->ci->config->item('autologin_cookie_name', 'tank_auth'), TRUE)) {
				...
				if (isset($data['key']) AND isset($data['user_id'])) {
					...
					if (!is_null($user = $this->ci->user_autologin->get($data['user_id'], md5($data['key'])))) {
						...
						// Renew usuarios cookie to prevent it from expiring
						set_cookie(array(
								'name' 		=> $this->ci->config->item('autologin_cookie_name', 'tank_auth'),
								'value'		=> $cookie,
								'expire'	=> $this->ci->config->item('autologin_cookie_life', 'tank_auth'),
    ...
    /**
     * busco el rol del usuarios por el id del usuarios
     *
     * @param	int
     * @return	string rol
     */
    function get_rol_by_user_id($user_id = null)
    {
        if ($user_id != null)  {
            return $this->ci->users->get_rol_by_user_id($user_id);
        }
        return null;
    }
}
