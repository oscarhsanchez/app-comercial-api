...
class captacion_incidencia_model extends CI_Model {
    ...
    private function getAssignedQuery($entityId, $userPk, $state, $lastTimeStamp) {
        ...
        $q = " SELECT DISTINCT incidencia.*
             ...
             AND (r_usu_cap.updated_at > '".$lastTimeStamp."' OR clientes.updated_at > '".$lastTimeStamp."' OR incidencia.updated_at > '".$lastTimeStamp."' )
    ...
    private function getAssignedRegistrosQuery($entityId, $userPk, $state, $lastTimeStamp) {
        ...
        $q = " SELECT DISTINCT registro_incidencia.*, CONCAT(usuarios.apellidos, CONCAT(', ',usuarios.nombre)) AS nombre_usuario
             ...
             WHERE clientes.bool_es_captacion = 1 AND clientes.fk_entidad = ".$entityId."
             AND (r_usu_cap.updated_at > '".$lastTimeStamp."' OR clientes.updated_at > '".$lastTimeStamp."' OR incidencia.updated_at > '".$lastTimeStamp."' OR registro_incidencia.updated_at > '".$lastTimeStamp."' )
             AND clientes.estado > 0  ";
    ...
    /**
     * Funcion que devuelve las incidencias asociadas a los clientes de captacion de un usuario,
    ...
    /**
     * Funcion que devuelve los registros de las incidencias asociadas a los clientes de captacion de un usuario,
     *  a partir de una fecha de actualizacion..
     * Las paginas generadas se cachean esperando las siguientes peticiones.
     *
     * @param $userPk
