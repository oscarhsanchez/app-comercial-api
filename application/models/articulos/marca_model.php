...
require_once(APPPATH.ENTITY_MARCA_ARTICULO);
...
class marca_model extends CI_Model {
    ...
    /**
     * Funcion que devuelve las marcas de una entidad, a partir de una fecha de actualizacion..
    ...
    function getMultipartCached($entityId, $pagination, $state, $lastTimeStamp) {
        ...
        if (isset($pagination->cache_token)) {
            ...
            $marcas = unserialize($this->esocialmemcache->get($key));
        } else {
            $this->db->where('fk_entidad', $entityId);
            $this->db->where('estado >=', $state);
            $this->db->where('updated_at >=', $lastTimeStamp);
            $query = $this->db->get('marca_articulo');

            $marcas = $query->result('marcaArticulo');

            $rowcount = sizeof($marcas);
            ...
            if ($rowcount > $pagination->pageSize) {
                $chunk_result = array_chunk($marcas, $pagination->pageSize);

                $marcas = $chunk_result[0];
        ...
        return array("pagination" => $pagination, "marcas" => $marcas?$marcas:array());
     ...
     */
    function marcaSearch($entityId, $field, $query, $return=null, $type='text') {
        if (!$return) {
            $return = 'pk_marca_articulo';
        }

        $this->db->select('DISTINCT '.$return.' AS value,'.$field.' AS description', false);
        $this->db->from('marca_articulo');
        $this->db->where('fk_entidad', $entityId);
