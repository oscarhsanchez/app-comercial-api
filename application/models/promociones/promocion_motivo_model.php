...
require_once(APPPATH.ENTITY_MOTIVO_PROMOCION);
require_once(APPPATH.ENTITY_R_PED_PRO);


class promocion_motivo_model extends CI_Model {

    /** ---------------------------------------
     *            FUNCIONES DE APOYO
    ----------------------------------------*/
    private function getQuery($entityId, $state, $lastTimeStamp) {
        $this->db->select('motivos_promocion.*');
        $this->db->from('motivos_promocion');
    ...
    /**
     * Devuelve una relacion motivo promocion / pedido a partir de su token
     *
     * @param $token
     * @return $r_ped_pro
     */
    function getRPedProByToken($token) {


        $this->db->where('r_ped_pro.token', $token);
        $query = $this->db->get('r_ped_pro');

        $r_ped_pro = $query->row(0, 'r_ped_pro');
    ...
    /**
     * Funcion que guardar la relacion entre una promocion y un pedido en la bbdd
     *
     * @param $r_ped_pro
     * @return bool
     * @throws APIexception
     */
    function saveRPedPro($r_ped_pro) {
        $this->load->model("log_model");

        if (!isset($r_ped_pro->token)) {
            $r_ped_pro->token = getToken();
        }

        $result = $r_ped_pro->_save(false, true);

        if ($result) {
            return true;
        } else {
            throw new APIexception("Error on promocion_motivo_model->saveRPedPro. Unable to update r_ped_pro.", ERROR_SAVING_DATA, serialize($r_ped_pro));
        }
    }
    ...
    /**
     *
     * Funcion que devuelve los motivos de promocion de una entidad
     ...
     * @return array($pagination, array($motivoPromocion))
     *
     *
     */
    function getMultipartCachedMotivos($entityId, $pagination, $state, $lastTimeStamp) {
        ...
        if (isset($pagination->cache_token)) {
            ...
            $motivos = $query->result('motivoPromocion');
